...
# ---------------- OUTLIER REMOVAL (BALANCE RANGE) ----------------
df = df[(df['balance'] >= 40000) & (df['balance'] <= 80000)]

st.write("Balance outliers removed (range: 40,000 to 80,000)")
...
balance = st.number_input("Balance", 40000, 80000, 50000)
...
if st.button("Predict"):
    ...
    if prediction == 1:
        st.write("Prediction: Customer is likely to subscribe")
    else:
        st.write("Prediction: Customer is not likely to subscribe")
